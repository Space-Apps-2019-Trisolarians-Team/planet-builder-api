...
def read_similar_exoplanets(pl_rade: float = 1, pl_distance: float = 1, first=10):
    return planetapi.similar_planets(pl_rade, pl_distance, first=first)


@app.get("/similar_exoplanet")
def read_similar_exoplanet(pl_rade: float = 1, pl_orbsmax: float = 1):
    return planetapi.similar_planets(pl_rade, pl_orbsmax, 1)[0]
